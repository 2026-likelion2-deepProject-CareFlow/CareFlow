// 파일 경로: src/main/java/com/careflow/engineer/service/EngineerDashboardService.java
...
public class EngineerDashboardService {
    ...
    private final BankAccountRepository bankAccountRepository; // v21 계좌 정보 추가
    ...
    public EngineerDashboardResponse getDashboardData(Long engineerId) {
        ...
        List<EngineerDashboardResponse.TodayScheduleDto> schedules = todayAssignments.stream().map(a -> {
            String fullAddress = (a.getAsRequest().getCustomer().getRegionId() != null ? a.getAsRequest().getCustomer().getRegionId().getName() + " " : "")
                    + a.getAsRequest().getVisitAddressDetail();

            return EngineerDashboardResponse.TodayScheduleDto.builder()
                    .assignmentId(a.getId())
                    .time(a.getAsRequest().getScheduledTime())
                    .status(a.getStatus()) // WAITING, ACCEPTED, COMPLETED
                    .productName(a.getAsRequest().getAppliance().getBrand() + " " + a.getAsRequest().getAppliance().getCategory().getName())
                    .modelNo(a.getAsRequest().getAppliance().getModelName())
                    .customerName(a.getAsRequest().getCustomer().getName())
                    .customerPhone(a.getAsRequest().getCustomer().getPhone())
                    .address(fullAddress.trim())
                    .symptom(a.getAsRequest().getSymptom().getSymptomName() + " - " + a.getAsRequest().getSymptomDesc())
                    .build();
        }).toList();
        ...
        List<EngineerDashboardResponse.NoticeDto> notices = notificationRepository.findByUser_IdOrderByCreatedAtDesc(engineerId, PageRequest.of(0, 3))
                .stream().map(n -> EngineerDashboardResponse.NoticeDto.builder()
                        .id(n.getId()).text(n.getTitle()).date(n.getCreatedAt().format(DateTimeFormatter.ofPattern("yyyy.MM.dd"))).build()
                ).toList();

        return EngineerDashboardResponse.builder()
                .engineerName(profile.getUser().getName())
    ...
    public EngineerSettlementSummaryResponse getSettlementSummary(Long engineerId, LocalDate dateFrom, LocalDate dateTo) {
        ...
        long rejectedCount = asAssignmentRepository.findByEngineer_IdAndStatus(engineerId, "REJECTED").stream()
                .filter(a -> !a.getAssignedAt().toLocalDate().isBefore(start) && !a.getAssignedAt().toLocalDate().isAfter(end))
                .count();
        ...
        for (AsAssignment a : completedAssignments) {
            ...
            if (a.getAsRequest().getReview() != null) {
                reviewCount++;
        ...
        Map<String, Long> brandMap = completedAssignments.stream()
                .collect(Collectors.groupingBy(a -> a.getAsRequest().getAppliance().getBrand(), Collectors.counting()));
        List<EngineerSettlementSummaryResponse.BrandDist> brandDists = brandMap.entrySet().stream()
                .map(e -> EngineerSettlementSummaryResponse.BrandDist.builder()
                        ...
                        .pct(String.format("%.1f%%", (e.getValue() / (double) totalCompleted) * 100))
                        .color("#818cf8").build()).toList();

        // 3-3. 상태별 분포 매핑 (프론트엔드 요구사항 준수)
        Map<String, Long> statusMap = completedAssignments.stream()
                .filter(a -> a.getAsRequest().getWorkReport() != null)
        ...
        // 3-4. 실적 리스트
        List<EngineerSettlementSummaryResponse.PerformanceItem> performanceList = completedAssignments.stream().map(a -> {
            String reqId = String.format("AS-%s-%04d", a.getAsRequest().getCreatedAt().format(DateTimeFormatter.ofPattern("yyyyMMdd")), a.getAsRequest().getId());
            return EngineerSettlementSummaryResponse.PerformanceItem.builder()
                    .requestId(reqId)
                    .workDate(a.getAsRequest().getScheduledDate().format(DateTimeFormatter.ofPattern("MM.dd")))
                    .customerName(a.getAsRequest().getCustomer().getName())
                    .productName(a.getAsRequest().getAppliance().getBrand() + " " + a.getAsRequest().getAppliance().getModelName())
                    .brand(a.getAsRequest().getAppliance().getBrand())
                    .grossAmount(a.getAsRequest().getWorkReport() != null ? a.getAsRequest().getWorkReport().getFinalAmount() : 0)
                    .diagnosisResult(a.getAsRequest().getWorkReport() != null ? a.getAsRequest().getWorkReport().getDiagnosisResult().name() : "NORMAL")
                    .rating(a.getAsRequest().getReview() != null ? a.getAsRequest().getReview().getRating() : 0.0)
                    .build();
        }).toList();
        ...
        EngineerSettlementSummaryResponse.SettlementSummary settlementSummary = EngineerSettlementSummaryResponse.SettlementSummary.builder()
                ...
                .bankName(bankAccount != null ? bankAccount.getBankName() : "미등록")
                .accountNumber(bankAccount != null ? bankAccount.getAccountNumber() : "계좌 미등록")
                .build();
