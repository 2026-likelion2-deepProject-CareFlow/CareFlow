...
- **설명**: 수리 기사가 작업 완료 보고서를 제출하면(`POST /api/engineer/work-reports`), **동일 트랜잭션 내**에서 대상 가전의 진단서를 자동 갱신한다. 별도 REST 엔드포인트가 아닌 도메인 규칙으로 동작한다. (보고서 제출 흐름은 `workCompletionReport.md` 참조)
