...
- **참고 (✨변경)**: 해당 날짜 근무표가 없어도 **예외를 던지지 않고** 안전한 휴무 응답(`ScheduleResponse.ofOffDay(date)` — `scheduleId=null`, `status="OFF"`, `timeSlots=[]`)을 `200 OK` 로 반환한다. (구: 근무표 없으면 `400` → 프론트가 안전하게 휴무로 인식하도록 개선됨)

### [DELETE] /api/engineer/my-schedule/{scheduleId} - deleteSchedule
- **설명**: 근무표를 삭제한다. **현재 구현은 근무표 행을 실제로 제거하는 물리 삭제(`repository.delete`)** 이며, 슬롯은 Cascade/`orphanRemoval` 로 함께 삭제된다. **이미 A/S가 배정된(`BOOKED`) 근무표는 삭제할 수 없다.** (응답 문자열은 안내상 "휴무(OFF) 처리"로 표기되나, 실제로는 행이 삭제되어 같은 날짜에 재등록이 가능하다.)
...
3. **응답(Response) 단계** : HTTP `201`, `ScheduleResponse.of(...)` 반환
...
- `findByUser_IdAndWorkDateBetweenOrderByWorkDateAsc(userId, date, date)` 로 해당 날짜만 조회 후 첫 건을 `ScheduleResponse.of(...)` 로 변환
- **결과가 없으면 예외가 아니라 `ScheduleResponse.ofOffDay(date)`(휴무 응답)를 반환**(`orElseGet`) — `200 OK`

### (4) [DELETE] 근무표 삭제(OFF 처리) — deleteSchedule
1. **검증** : 근무표 존재 확인, **본인 소유 검증**(`schedule.user.id == userId`)
2. **상태 검증** : `BOOKED`(배정됨) 상태면 삭제 불가
3. **처리** : `engineerScheduleRepository.delete(schedule)` — **근무표 행 물리 삭제**(슬롯은 Cascade/orphanRemoval 로 함께 제거). 같은 날짜 재등록이 가능하도록 하기 위한 설계.
4. **응답** : HTTP `200`, 안내 문자열(`"해당 날짜의 근무표가 휴무(OFF) 처리되었습니다."`) 반환
...
- 주요 예외 매핑
    ...
    - **일간 조회 시 해당 날짜 근무표 없음 → `200 OK` + 휴무(OFF) 응답** (예외 아님, `ofOffDay`)
    - 타인의 근무표 삭제 시도 → `400`
...
- 응답 DTO 는 정적 팩토리 `of()` 로 변환 (근무표 없는 날짜용 `ofOffDay()` 별도 제공)
