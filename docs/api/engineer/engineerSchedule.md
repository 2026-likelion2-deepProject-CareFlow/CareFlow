...
### [POST] /api/engineers/me/schedules - EngineerScheduleController.createSchedule
...
### [GET] /api/engineers/me/schedules?year={year}&month={month} - getMonthlySchedules
- **설명**: 본인의 월간 근무 일정을 조회한다(근무 날짜 오름차순).
- **Query Params**: `year`(int, 필수), `month`(int, 필수)
- **Response (200 OK)**: `List<ScheduleResponse>`

### [DELETE] /api/engineers/me/schedules/{scheduleId} - deleteSchedule
...
### (3) [DELETE] 근무표 삭제(OFF 처리) — deleteSchedule
...
- 등록/삭제 로직을 **하나의 `@Transactional`** 안에서 수행하되, 도중 DB 에러 발생 시 모든 변경사항을 롤백할 것
- 주요 예외 매핑
